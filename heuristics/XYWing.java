...
public class XYWing extends AHeuristic implements IHeuristic {
	
	public XYWing(Grid grid) {
		super(IHeuristic.LevelHeuristics.XYWing, grid);
	}
						...
						for (Cell cellSecond : mapSecond.keySet()) {
							...
							if (valueFirst.equals(valueSecond)) {
								...
								if (remove.getCandidates() != null &&
									...
									setSolution((StdSolution) sol, cell, cellFirst, cellSecond, remove, valueFirst);
									return sol;
								}
							}
						}
	...
	private void setSolution(StdSolution solution, Cell beginning, Cell first, Cell second, Cell remove, String value) {
		try {
			solution.addReason(Color.BLUE, beginning);
			solution.addReason(Color.BLUE, first);
			solution.addReason(Color.BLUE, second);
			solution.addReason(Color.ORANGE, remove);
			solution.addAction(new EliminateCandidate(remove, value));
	...
	private StringBuilder getDescription(Cell beginning, Cell first, Cell second, Cell remove, String value) {
		...
		sb.append(value + " dans la case finale (en Orange).\n");
